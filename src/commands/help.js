    ...
    async execute(message, args) {
        ...
        if (args[0]) {
            ...
            if (cmd) {
                const embed = {
                    color: 0x0099ff,
                    title: `📖 Lệnh: ${BotConfig.PREFIX}${cmd.name}`,
                    fields: [
                    ...
                    footer: { text: `${BotConfig.BOT_NAME} v${BotConfig.BOT_VERSION}` }
                };
        ...
        const embed = {
            color: 0x7289da,
            title: `🤖 ${BotConfig.BOT_NAME} - Lệnh`,
            description: `Prefix: \`${BotConfig.PREFIX}\``,
            fields: commands.map(cmd => ({
                name: `${BotConfig.PREFIX}${cmd.name}`,
                value: `${cmd.desc}\n\`${cmd.usage}\``,
                inline: false
            })),
            footer: { 
                text: `Dùng ${BotConfig.PREFIX}help <lệnh> để xem chi tiết | ${BotConfig.BOT_NAME} v${BotConfig.BOT_VERSION}`
            }
        };
